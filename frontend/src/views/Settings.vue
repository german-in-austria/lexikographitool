...
<v-col cols="12">
     ...
     <div class="text-h7 py-1">Meine Orte</div>
            ...
            <settings-location-multiple v-model="locations" @inputFav="location=$event.value"></settings-location-multiple>
          </v-col>
          ...
          <v-col class="d-flex ml-auto" cols="12" sm="3" xsm="12">
           ...
           <v-col cols="12">
                     <v-switch v-model="showSensitiveData" label="zeige mir sensible Wörter an"></v-switch>
...
name: "Settings",
  ...
  data: () => ({
    valid:true,
    location:'',
    locations:[{ value: { id: -1, zipcode: "", place: "" } }],
    age: 21,
    email: "",
    password: "",
    username: "",
  ...
  created() {
    axios.get('account/me/').then(response=>{
      ...
      this.locations = this.locations.map(val=>{return {'value':val}})
      this.locations = [{value:this.location}].concat(this.locations)
      this.locations=this.locations.filter((v,i,a)=>a.findIndex(t=>(t.value.id === v.value.id))===i)
  ...
  save (){
    if (this.$refs.registerForm.validate()) {
      const loc = this.locations.map(({value})=>value);
      console.log(loc)
      RequestHandler.updateAccount({'email':this.email, 'username':this.username,'home':this.location,'age':18,'locations':loc,'show_sensitive_words':this.showSensitiveData})
