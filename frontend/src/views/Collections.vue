...
" >mdi-plus</v-icon></v-card-actions>
      ...
      <v-col cols="12" sd="4" md="4"  v-for="collection in collections"
             :key="collection.id" >
             <v-hover v-slot="{ hover }">
        <v-card
...
export default {
  ...
  methods: {
    createCollection(){
      RequestHandler.createCollection(new Collection(this.collectionTitle)).then(
        (response) => this.collections.push(response.data)
      )
      this.dialog = false
