  ...
  <div>
    <v-form
      ...
      <v-container>
        <v-row>
          <v-col>
            <v-text-field v-model="dialectWord" label="Dialektwort" :rules="[v => !!v || 'Dialektwort muss angegeben werde']" required></v-text-field>
          </v-col>
        </v-row>

        <v-row>
          <v-col>
            <CardCreateLexem @loadLexeme="loadLexeme" @inputData="newLexeme" :rules="[v => !!v || !!description || 'Hauptlexeme oder Beschreibung muss angegeben werden']"></CardCreateLexem>
          </v-col>
          <v-col>
            <CardCreateDescription @inputData="newDescription" :rules="[v => !!v || !!lexeme || 'Hauptlexeme oder Beschreibung muss angegeben werden']"></CardCreateDescription>
<!--            <v-text-field label="Beschreibung" v-model="description"></v-text-field>-->
        ...
        <v-row>
          <v-col>
            <card-create-button-group :selected="kind" @inputData="newKind" :items="kinds"></card-create-button-group>
            <card-create-category :selected="categories" @inputData="newCategory">></card-create-category>
          </v-col>
        </v-row>
        ...
        <v-row>
          <v-col>
            <card-create-button-group :selected="type" @inputData="newType" :items="kindItems"></card-create-button-group>
          </v-col>
          <v-col>
            <CardCreateDialect :disabled="type =='youth-language'" @inputData="newDialect"></CardCreateDialect>

            <!--            <v-text-field label="Welcher Dialekt ist das?" v-model="dialect" :disabled="type =='youth-language'"></v-text-field>-->
          </v-col>
        </v-row>
            <card-create-multiple-dropdown label="Etymologie (Wortherkunft)" @inputData="newEtymology"></card-create-multiple-dropdown>
            <card-create-multiple-dropdown label="Aussprache" @inputData="newPronunciation"></card-create-multiple-dropdown>
            <card-create-multiple-dropdown label="Beispiel" @inputData="newExample"></card-create-multiple-dropdown>
...
export default {
  ...
  components: {
    ...
    CardCreateCategory,
    CardCreateMultipleDropdown, CardCreateButtonGroup, CardCreateLexem, CardCreateLocation
  },
  data: () => ({
    valid: false,
    lexeme: '',
  ...
  methods: {
    async createNewLexeme() {
      if(this.type == 'youth-language')
        this.dialect = 'Jugendsprache'
      var categories = []
      this.categories.forEach( item => categories.push( new Category(item)))
      var lexemeId
      var lexeme = new Lexeme(this.lexeme,this.description,this.dialectWord,this.kind,this.dialect, this.origin.id)
      requestHandler.createDialect(new Dialect(this.dialect))
    ...
    async loadLexeme() {
      ...
      requestHandler.getFirsLexemeByName(this.lexeme)
          .then(response => {
            this.categories=[]
            response.data.categories.forEach(c => {
              this.categories.indexOf(c.category) === -1 ? this.categories.push(c.category) : console.log("This item already exists")
            })
...
<style scoped>
.v-text-field.v-text-field--solo .v-input__control { min-height: 56px; }
