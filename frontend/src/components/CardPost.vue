<template>
  ...
  >
      ...
      <v-hover v-slot="{ hover }">
        ...
        >
          ...
          >
            ...
            <p class="text--primary"> <v-icon> mdi-account</v-icon>{{ post.author.username }}</p>
            <p style="margin-left:5px; margin-top: -15px" class="text--accent-1 text--secondary">{{dateCreated}}</p>
          </v-card-subtitle
          >
          <v-card-text v-if="!edit" >
            <p class="text-body-1">{{ post.text }}</p>
            <p style="margin-top: -18px" class="text--disabled" v-if="post.edited">bearbeitet</p>
          </v-card-text>
          <v-card-text v-else>
            <v-textarea rows="2" solo flat dense v-model="newText" @keyup.enter="updatePost"></v-textarea>
          </v-card-text>
          <v-card-actions>
            ...
            >
              ...
              <v-list>
                <v-list-item v-if="post.is_author" @click="newText=post.text;edit=!edit">{{$t('postings.edit')}}</v-list-item>
                <v-list-item v-if="post.is_author" @click="deletePost">{{$t('postings.delete')}}</v-list-item>
                <v-list-item><report-dialog :item="post" kind="post"></report-dialog></v-list-item>
    ...
    <v-card
      ...
      <v-card-text>
        <p  v-if="!!children && children.length == 0" v-html='$t("postings.noCommentMessage",{user: post.author.username})'></p>
...
export default {
  ...
  watch: {
    dialog() {
      Axios.get('posts/' + this.post.id + '/').then(response => this.children = response.data)
